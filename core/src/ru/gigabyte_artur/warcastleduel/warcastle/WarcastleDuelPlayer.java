...
public class WarcastleDuelPlayer extends Player
{
    ...
    public static final int MAX_CARDS_HAND = 12;              // Максимальное число карт в руке.
...
//        this.getPrivateHand().DrawCard(this.getDeck());
    ...
    /** Считывает карты текущего игрока из элемента XML Element_in */
    private void ReadCardsFromXml(Element Element_in)
    {
        // Инициализация.
        String GuidText, CodeNameText, SellPriceText;
        Integer NewSellPrice = 0;
        Hand PlayerPrivateHand = getPrivateHand();
        // Чтение XML
        Element CardsElement = WcnPackedMessage.ExtractElementByPathFromElement(Element_in, "Player->Hand");
        if (CardsElement != null)
        {
            List<Element> CardsElements = CardsElement.getChildren();
            for (Element Curr_CardElements : CardsElements)
            {
                ...
                CodeNameText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Curr_CardElements, "CodeName");
                SellPriceText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Curr_CardElements, "SellPrice");
                NewSellPrice = Integer.parseInt(SellPriceText);
                if (!PlayerPrivateHand.FindCardByGUID(GuidText))
                {
                    // Добавление новых карт.
                    // PlayerPrivateHand.Empty();
                    WarcastleDuelCard NewCard = WarcastleDuelCard.GenerateCardByCodeName(CodeNameText);
                    NewCard.setGUID(GuidText);
                    NewCard.setSellPrice(NewSellPrice);
                    PlayerPrivateHand.AddCard(NewCard);
                }
                else
                {
                    // Такая карта уже есть у игрока.
                }
                // TODO: Очистка незадействованных карт.
            }
        }
        else
        {
            System.out.println("Cannot read cards from XML file");
        }
    }

    /** Считывает расширенные статы текущего игрока из элемента XML Element_in.*/
    public void FillPlayerByElement(Element Element_in)
    {
        String LastText;
        // Имя и GUID.
        LastText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Element_in, "Player->Name");
        this.setName(LastText);
        LastText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Element_in, "Player->GUID");
        this.setGUID(LastText);
        // Броня и золото.
        LastText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Element_in, "Player->Amount");
        this.setAmount(WcnXmlBuilder.StringToInteger(LastText));
        LastText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Element_in, "Player->Armor");
        this.setArmor(WcnXmlBuilder.StringToInteger(LastText));
        // Мораль.
        LastText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Element_in, "Player->Morale");
        this.setMorale(WcnXmlBuilder.StringToInteger(LastText));
        // Статы.
        LastText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Element_in, "Player->Swords");
        this.setSwords(WcnXmlBuilder.StringToInteger(LastText));
        LastText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Element_in, "Player->Priests");
        this.setPriests(WcnXmlBuilder.StringToInteger(LastText));
        LastText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Element_in, "Player->Instructors");
        this.setInstructors(WcnXmlBuilder.StringToInteger(LastText));
        LastText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Element_in, "Player->Peasants");
        this.setPeasants(WcnXmlBuilder.StringToInteger(LastText));
        LastText = WcnPackedMessage.ExtractStringByXmlPathFromElement(Element_in, "Player->Horses");
        this.setHorses(WcnXmlBuilder.StringToInteger(LastText));
        // Карты.
        ReadCardsFromXml(Element_in);
    }

    /** Считывает статы игрока 1 из сообщения XML*/
    public static WarcastleDuelPlayer ReadPlayer1FromXML(String Message_in)
    {
        WarcastleDuelPlayer rez = new WarcastleDuelPlayer();
        Element Player1Element = WcnPackedMessage.ExtractElementByXmlPath(Message_in, "Body->Player1");
        rez.FillPlayerByElement(Player1Element);
        return rez;
    }

    /** Считывает статы игрока 2 из сообщения XML*/
    public static WarcastleDuelPlayer ReadPlayer2FromXML(String Message_in)
    {
        WarcastleDuelPlayer rez = new WarcastleDuelPlayer();
        Element Player1Element = WcnPackedMessage.ExtractElementByXmlPath(Message_in, "Body->Player2");
        rez.FillPlayerByElement(Player1Element);
        return rez;
    }

    /** Увеличивает у текущего игрока стат с ID StatId_in на значение Value_in, проверяя сумму на счёте.*/
    public void CheckIncrementStat(int StatId_in, int Value_in)
    {
        int Price;
        Price = PriceStatById(StatId_in);
        if (Price <= getAmount())
        {
            IncrementStatById(StatId_in, Value_in);
            IncrementAmount(-Price);
        }
        else
        {
